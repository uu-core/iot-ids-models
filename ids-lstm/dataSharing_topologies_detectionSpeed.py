...
parser = argparse.ArgumentParser(description = "Detection accuracy over tiime: network topologies (shallow, medium, deep)")
...
for run in range(runs):
   ...
   medium_Train, medium_Test = aggregate.aggregate_list_oo()
   deep_Train, deep_Test = aggregate.aggregate_list_deep()

   all_Train = shallow_Train + medium_Train + deep_Train
   all_Test = shallow_Test + medium_Test + deep_Test
   ...
   # Normalize Base test
   shallow_normalized_dfs_test = [df.apply(lambda x: (x - all_min[x.name]) / (all_max[x.name] - all_min[x.name])) for df in shallow_Test]
   transition_indices_shallow = [df['label'].apply(lambda x: x == 1).idxmax() for df in shallow_normalized_dfs_test]
   len_shallow = [df.shape[0] for df in shallow_normalized_dfs_test]
   
   # Normalize OO test
   medium_normalized_dfs_test = [df.apply(lambda x: (x - all_min[x.name]) / (all_max[x.name] - all_min[x.name])) for df in medium_Test]
   transition_indices_medium = [df['label'].apply(lambda x: x == 1).idxmax() for df in medium_normalized_dfs_test]
   len_medium = [df.shape[0] for df in medium_normalized_dfs_test]

   # Normalize DEC test
   deep_normalized_dfs_test = [df.apply(lambda x: (x - all_min[x.name]) / (all_max[x.name] - all_min[x.name])) for df in deep_Test]
   transition_indices_deep = [df['label'].apply(lambda x: x == 1).idxmax() for df in deep_normalized_dfs_test]
   len_deep = [df.shape[0] for df in deep_normalized_dfs_test]
   ...
   sequencedshallow_Test = [seqMaker.seq_maker(df,10) for df in shallow_normalized_dfs_test]
   sequencedmedium_Test = [seqMaker.seq_maker(df,10) for df in medium_normalized_dfs_test]
   sequenceddeep_Test = [seqMaker.seq_maker(df,10) for df in deep_normalized_dfs_test]
   ...
   shallowCoojaRuns = [dataPreparation(df) for df in sequencedshallow_Test]
   mediumCoojaRuns = [dataPreparation(df) for df in sequencedmedium_Test]
   deepCoojaRuns = [dataPreparation(df) for df in sequenceddeep_Test]

   lenBASECoojaRuns = [len(dl.dataset) for dl in shallowCoojaRuns]
   lenOOCoojaRuns = [len(dl.dataset) for dl in mediumCoojaRuns]
   lenDECCoojaRuns = [len(dl.dataset) for dl in deepCoojaRuns]

   
   shallowCoojaRunPrediction = [timeFinder(dl,trans) for dl, trans in zip(shallowCoojaRuns, transition_indices_shallow)]
   mediumCoojaRunPrediction = [timeFinder(dl,trans) for dl, trans in zip(mediumCoojaRuns,transition_indices_medium)]
   deepCoojaRunPrediction = [timeFinder(dl,trans) for dl, trans in zip(deepCoojaRuns,transition_indices_deep)]



   with open(add + '/detect_time_all_shallow.csv', 'w',newline = '') as file:
      writer = csv.writer(file)
      writer.writerow(shallowCoojaRunPrediction)

   with open(add + '/detect_time_all_medium.csv', 'w',newline = '') as file:
      writer = csv.writer(file)
      writer.writerow(mediumCoojaRunPrediction)

   with open(add + '/detect_time_all_deep.csv', 'w',newline = '') as file:
      writer = csv.writer(file)
      writer.writerow(deepCoojaRunPrediction)
